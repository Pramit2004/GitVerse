...
export default function Navbar({ readingProgress = 0 }) {
  ...
  return (
    <>
      {/* Reading progress bar — only in chapter view */}
      ...
      <nav style={{ position:'sticky', top:0, zIndex:100, background:'rgba(10,14,26,0.96)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', borderBottom:'1px solid var(--border)', padding:'0 clamp(12px,3vw,24px)' }}>
        <div style={{ maxWidth:'var(--page-max)', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:'var(--nav-height)', gap:10 }}>
          ...
          {/* Breadcrumb — desktop, chapter view only */}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:7, flexShrink:0 }}>
            ...
            <button onClick={() => setSearchOpen(true)} aria-label="Search (press /)"
              style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-muted)', borderRadius:'var(--radius-md)', padding: isMobile ? '6px 10px' : '6px 13px', fontFamily:'var(--font-mono)', fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:5, transition:'all 0.15s' }}>
              <span>🔍</span>
            ...
            {/* Progress ring */}
            <div onClick={() => setView('all-chapters')} title={`${completedCount}/18 chapters complete`}
